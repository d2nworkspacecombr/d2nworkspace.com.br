...
async function confirmarAdmin(supabase) {
  const { data: auth } = await supabase.auth.getUser();
  if (!auth?.user) return { erro: "Não autenticado", status: 401 };
  const { data: perfil } = await supabase.from("usuarios").select("role").eq("id", auth.user.id).single();
  if (!perfil || perfil.role !== "admin") return { erro: "Só o administrador pode fazer isso.", status: 403 };
  return { usuarioId: auth.user.id };
}
...
module.exports = async (req, res) => {
  ...
  if (req.method === "PATCH") {
    ...
    const checagem = await confirmarAdmin(supabase);
