// lib/colaboradores-criar.js (antigo api/colaboradores-criar.js)
...
const { clienteComoUsuario, clienteAdmin, exigirAdmin, enviarErro } = require("./supabase");
...
module.exports = async (req, res) => {
  try {
    ...
    const checagem = await exigirAdmin(supabaseUsuario);
    ...
    if (!process.env.SUPABASE_SERVICE_KEY) {
      return enviarErro(
        res,
        500,
        "Faltou configurar a variável SUPABASE_SERVICE_KEY no Vercel (ou o nome dela está diferente)."
      );
    }
    ...
    if (Array.isArray(cliente_ids) && cliente_ids.length > 0) {
      const vinculos = cliente_ids.map((cliente_id) => ({
        ...
        modulo: "assessoria",
        vinculado_por: checagem.usuarioId,
      }));
      const { error: erroVinculo } = await admin.from("vinculo_colaborador_cliente").insert(vinculos);
      if (erroVinculo) {
        // o login já foi criado; avisa mas não derruba a resposta
        return res.status(201).json({
          ok: true,
          usuario_id: novoUsuario.user.id,
          aviso: "Colaborador criado, mas houve um problema ao vincular os clientes: " + erroVinculo.message,
        });
      }
    }
    ...
    enviarErro(res, 500, "Erro inesperado no servidor: " + (err.message || String(err)));
  }
};
