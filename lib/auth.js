// lib/auth.js (antigo api/auth.js)
// Reúne login e renovação de sessão num arquivo só.
//
...
const { clientePublico, enviarErro } = require("./supabase");
  ...
  try {
    ...
    if (acao === "login") {
      ...
      const { data: usuario, error: erroPerfil } = await supabaseAutenticado
        .from("usuarios")
        .select("id, nome, email, role, cargo, foto_url")
        .eq("id", data.user.id)
        .single();
