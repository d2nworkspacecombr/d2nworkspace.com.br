...
// Postgres/Supabase mandam detalhe, dica e código junto da mensagem principal
// do erro — útil pra diagnosticar sem precisar adivinhar. Mostramos tudo.
function descreverErro(error) {
  const partes = [error.message];
  if (error.details) partes.push("detalhes: " + error.details);
  if (error.hint) partes.push("dica: " + error.hint);
...
module.exports = async (req, res) => {
  ...
  if (erroAuth || !auth?.user) return enviarErro(res, 401, "Não autenticado: " + descreverErro(erroAuth || {}));
  ...
  if (req.method === "GET") {
    ...
    if (cliente_id) {
      ...
      if (error) return enviarErro(res, 500, descreverErro(error));
    ...
    if (erroClientes) return enviarErro(res, 500, descreverErro(erroClientes));

    const { data: briefings, error: erroBriefings } = await admin
      .from("briefing")
      .select("cliente_id");

    if (erroBriefings) return enviarErro(res, 500, descreverErro(erroBriefings));
  ...
  if (req.method === "POST") {
    ...
    if (error) return enviarErro(res, 400, descreverErro(error));
