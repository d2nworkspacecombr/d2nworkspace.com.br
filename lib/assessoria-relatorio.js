...
function statusPorPercentual(p) {
  const n = Number(p) || 0;
  ...
  (dados.okrs || []).forEach((okr) => {
    (okr.krs || []).forEach((kr) => {
      ...
      desejado.push({
        chave: "kr:" + kr.id,
        titulo: kr.l,
        status: statusPorPercentual(kr.p),
        paiChave: null,
        responsavel_ids: [],
        descricao: null,
        prazo: null,
...
async function sincronizarAssessoria(admin, clienteNome, usuarioId, dados) {
  ...
  // Processa primeiro quem não tem "mãe" (atendimentos, KRs), depois os filhos
  // (tarefas operacionais), pra garantir que o tarefa_pai_id já exista.
